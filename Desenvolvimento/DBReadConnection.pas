...
begin
  ConfigIni := TIniFile.Create('./Config.ini');
    ...
    servidorDaBase        := ConfigIni.ReadString('Base de Dados','SERV','SERV não encontrado no ini');
    portaDoServidorDaBase := ConfigIni.ReadString('Base de Dados','PORT','PORT não encontrado no ini');
    tipoDaBase            := ConfigIni.ReadString('Base de Dados','TIPO','TIPO não encontrado no ini');
  ConfigIni.Free;
end;
